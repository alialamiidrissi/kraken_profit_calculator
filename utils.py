...
def get_pair_from_kraken(from_curr, to_curr, client, date=None):
    ...
    if (date is None) or (date == "latest"):
        ...
        res = retry_kraken(get_ohlc_from_kraken(client),
                           new_name, to_curr, interval=1440)
        return res
...
def retry_kraken(func, from_currency, to_currency, index_normalize=True, *args, **kwargs):
    """If a direct conversion is not available convert to USD first"""
    ...
    try:
        ...
        logging.debug(
            f"Failed to convert {from_currency} to {to_currency}: Trying to go through the USD")
        pair_1 = f"{from_currency}USD"
        pair_2 = f"USD{to_currency}"
        try:
